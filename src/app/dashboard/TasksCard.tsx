import { Button, Card, List, Skeleton, Popconfirm, message } from 'antd'
...
export default function TodosCard({ tasks, setTasks, setModalOpen, isLoading }: Props) {
  ...
  const getListItemActions = ({ _id, isComplete }: Task) => [
    <Button type={isComplete ? 'dashed' : 'dashed'} key="done" onClick={() => updateTaskStatus(_id, isComplete)}>
      {isComplete ? <UndoOutlined /> : 'Complete'}
    </Button>,
    <Popconfirm
            ...
            return (
              <List.Item actions={actions}>
                <Skeleton title={false} loading={isLoading} active>
                  <List.Item.Meta
                    title={<span className={isComplete ? styles.completedTask : ''}>{name}</span>}
                    description={<span className={isComplete ? styles.completedTask : ''}>{description}</span>}
                  />
                </Skeleton>
              </List.Item>
            )
